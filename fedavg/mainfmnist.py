...
from dataset.datasets.partitioned_femnist import PartitionedFMNIST
...
def arg_parse():
    ...
    parser.add_argument("--recordId", type=str, default="femnist_dirichlet_0.5_100_0.1")
    ...
    """
    parser.add_argument("--datasetName", type=str, default="fmnist", choices=["fmnist"])
    parser.add_argument("--partition", type=str, default="noniid-#label",
                        choices=["noniid-#label", "noniid-labeldir", "iid"])
    ...
    parser.add_argument("--model", type=str, default="CnnWithBatchFormer",
                        choices=["CnnWithBatchFormer", "CnnWithFusion"])
    parser.add_argument("--modelConfig", type=dict,
                        default={"backbone": "FedAvgCNN", "modelConfig": {"dataset": "fmnist"}})
...
def run(configs, data_preprocessed=True):
    dataPartitioner = PartitionedFMNIST(numClients=configs.numClients, dataName=configs.datasetName,
                                       path="./clientdata/fmnist", data_preprocessed=data_preprocessed, download=True,
    ...
    if data_preprocessed:
        draw(dataPartitioner, "./clientdata/fmnist/datainfo")
...
if __name__ == '__main__':
    ...
    #
    # configs.model = "CnnWithFusion"
    # configs.modelConfig = {"backbone": "FedAvgCNN", "modelConfig": {"dataset": "fmnist"}}
    # for i in range(10):
    #     configs.recordId = f"CnnWithFusion[multi]_fmnist_noniid-#label_2_100_0.1_{i}"
    #     run(configs, data_preprocessed=True)

    configs.model = "CnnWithBatchFormer"
    configs.modelConfig = {"backbone": "FedAvgCNN", "modelConfig": {"dataset": "fmnist"}}
    for i in range(10):
        configs.recordId = f"CnnWithBatchFormer[10chanel]_fmnist_noniid-#label_2_100_0.1_{i}"
        run(configs, data_preprocessed=True)
